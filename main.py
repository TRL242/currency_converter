...
def converter():
	currency_1 = str(home_entry.get())
	currency_2 = str(conversion_entry.get())
	try:
	# Live currency update
		conversion_rate = (c.convert(1, currency_1, currency_2))
...
# convert to label
conversion_label = Label(conversion, text="Symbol of Currency To Convert To...")
...
# Rate To Entry
rate_entry = Label(conversion, text="Click Conversion Rate Button", font=("Helvetica", 24))
...
lock_button.grid(row=0, column=1, padx=10)
...
#convertion rate button
calculate_rate_button = Button(button_frame, text="Conversion Rate", command=converter)
calculate_rate_button.grid(row=0, column=0, padx=10)
